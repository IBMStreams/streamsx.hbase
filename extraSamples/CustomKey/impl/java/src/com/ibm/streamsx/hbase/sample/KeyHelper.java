...
public class KeyHelper {
	...
	/**
	 * A custom key generation function.  
	 * Here our key is a concatenation of a timestamp (represented as a long) and a location.
	 * (NOTE: This is probably not a good key, as consequtive updates will go to the same 
	 * HBase tablet server, and your application won't be able to get much parallelism.) 
	 * 
	 * This function will generate two spl functions:
	 * list<uint8> makeKey(uint64, T)
	 * list<int8> makeKey(int64,T).
	 * 
	 ...
	 * 
	 * The type list<uint8> can be cast to a blob easily (see the SPL code), but the type
	 * list<int8> can't, so it's probably a good idea to use unsigned types to generate keys.
	 * 
