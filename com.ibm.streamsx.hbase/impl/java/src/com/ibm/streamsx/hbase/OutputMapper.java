...
public abstract class OutputMapper {
	...
	/**
	 * This function checks that the type listType
	 * * is a list
	 * * is a list of type tuple
	 * * each tuple is one of the allowed basic types followed by a long
	 * 
	 * @param listType The type of the supposed list. 
	 * @param attrName  The name of the attribute--useful for logging only.  
	 * @return the schedule for the tuple.  The caller needs to store this to make tuples. 
	 * @throws Exception Throws an exception if listType isn't a suitable type.
	 */
	protected static StreamSchema checkListType(Type listType,String attrName) throws Exception {
		...
		if (tupSchema.getAttributeCount() != 2) {
			throw new Exception("Attribute "+attrName+" has list type with wrong number of attributes.  Found "+tupSchema.getAttributeCount()+" expected 2");
		}
		// if the second attribute isn't a long, don't know what to do.
		if (tupSchema.getAttribute(1).getType().getMetaType() != MetaType.INT64) {
			throw new Exception("Attribute "+attrName+" has list type, expected second attribute to be of type int64 for timestamp");
		}
		...
		if (!isAllowedBasicType(elementMetaType)) {
			throw new Exception("Attribute "+attrName+" has list type, expected first attribute to be of type int64, rstring, or blob");
		}
		return tupSchema;
	}
	
	/**
	 * This is a preliminary check that the type is one that can be handled by createOutputMapper.  
	 * This does only a superficial examination of the type--even if true,
	 * createOutputMapper may still throw an exception.
	 * @param mType The meta type of the field.
	 * @return
	 */
	...
	/**
	 * If true, populates from as single cell.  The caller must already have narrowed the data to a single column family
	 * and column qualifier.  Calling populateRecord will throw and exception.
	 * @return
	 */
	...
	protected List<Tuple> makeTupleList(StreamSchema tupleSchema, MetaType valueType, NavigableMap<Long,byte[]> fromHBASE) {
