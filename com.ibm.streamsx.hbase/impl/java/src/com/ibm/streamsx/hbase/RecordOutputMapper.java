...
public class RecordOutputMapper extends OutputMapper{
	 ...
	 */
	protected RecordOutputMapper(int _attrIndex, Type thisType,Charset _charset) throws Exception {
		...
		for (int i =0 ; i < numAttr;i++) {
			...
			if (isAllowedBasicType(mType)) {
				...
				outTypes[i] = tupleSchema[i].getAttribute(0).getType().getMetaType();
			}
			else {
	 ...
	 * 
	 * @param tuple Output tuple to be populated
	...
	public int populateRecord(OutputTuple tuple,
		...
		Map<String,Object> inMap = new HashMap<String,Object>(attrNames.length);
			...
			for(int i = 0; i < columnQualifiers.length; i++) {
				...
				if (resultMap.get(fam).containsKey(colQ)) {
					...
					if (tupleSchema[i] == null) {
						// easy to populate!	
						inMap.put(attrNames[i], castFromBytes(lastestEntry.getValue(),outTypes[i],charset));
					}
					else {
						inMap.put(attrNames[i], makeTupleList(tupleSchema[i],outTypes[i],resultMap.get(fam).get(colQ)));
					}
				}
			}
		}
